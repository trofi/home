...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
    ...
    , ((modMask     ,          xK_grave ), spawnx myTerminal)
...
------------------------------------------------------------------------
xmobarCmdT :: String
xmobarCmdT = "                         xmobar -o -B '#122c80' -F '#adbadd' -f 'xft:xos 4 Terminus-12' -t '%StdinReader%'"
xmobarCmdB :: String
xmobarCmdB = "~/bin/player-status.sh | xmobar -b -B '#122c60' -F '#adbadd' -f 'xft:xos 4 Terminus-10' -t '%StdinReader% }{ %battery% | %cpu% | %memory% ||| %wl0% ||| <fc=cyan>%date%</fc>'"
